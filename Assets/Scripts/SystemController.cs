...
public class SystemController : MonoBehaviour
{
    ...
    async void Awake()
    {
        ...
        data.history = "Early morning, " + name + " and User always spend time together.\n";
        ...
        forSummary.Add(new ChatGPTMessageModel(){role = "system", content = "You are a popular anime creator. Please summarize the given scene concisely. Additionaly, hink of an introduction to the next scene that follows. Also, consider the location it takes place with key item in the next scene. The two is always together, not apart. This conversation transitions in the order of waking up, breakfast, morning, lunch, afternoon, dinner, night. One happening that reveals "+data.name+"'s lovely aspect is needed. Out put with JSON style in Japanese. Represent 'User' by the word 'あなた'. \nWorld config: " + data.worldConfig + "\nex){\"summary\": \"説明\", \"nextscene\": \"説明\", \"location\": \"場所\", \"keyitem\": \"item1, item2, ......\"}"});
        UnityEngine.Debug.Log("SystemController initialized.");
        data.worldConfig = "Life in 2000s. Real world.";
    }
    async void Start()
    {
        //画像プロンプトの初期化
    ...
    public void WriteLog(string st)
    {
        var fileName = "./Assets/Resources/Log.txt";
        try
        {
    ...
    public async UniTask SceneTransition()
    {
        ...
        UnityEngine.Debug.Log("location: " + summary.location);
        UnityEngine.Debug.Log("keyitem: " + summary.keyitem);
        WriteLog("summary: " + summary.summary);
        WriteLog("nextscene: " + summary.nextscene);
        WriteLog("location: " + summary.location);
        WriteLog("keyitem: " + summary.keyitem);
        /*
        ...
        */
        ...
        forBackground.Add(new ChatGPTMessageModel(){role = "user", content = "World config: " + data.worldConfig + "\nlocation: " + summary.location + "\n next scene: " + summary.nextscene + "\n key item: " + summary.keyitem});
        string resPrompt = await ChatHandler.instance.RequestChatGPT(forBackground);
        forBackground.RemoveAt(1);
        BackgroundPromptModel bgPrompt = JsonUtility.FromJson<BackgroundPromptModel>(resPrompt);
        data.backgroundPrompt = bgPrompt.prompt;
        WriteLog("backgroundprompt: " + bgPrompt.prompt);

        await BackgroundGenerator.instance.RequestDALLE("DO NOT PAINT HUMANS. ONLY LANDSCAPE. " + data.backgroundPrompt);//Japanese anime style, like Japanese artist わいっしゅ. 
        ImageHandler.instance.ChangeBackground();
        uiText.text = "";
    ...
    public async UniTask CharacterAction(string response)
    {
        ...
        WriteLog("character: " + reaction.quote);
        //軽微な変更
